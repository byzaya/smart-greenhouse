...
module.exports = class TunnelCluster extends EventEmitter {
  ...
  open() {
    ...
    const localAddress = opt.local_address || localHost;
    const localPort = opt.local_port;
    const localProtocol = opt.local_https ? 'https' : 'http';
    const allowInvalidCert = opt.allow_invalid_cert;
    const realIp = opt.real_ip;

    debug(
      'establishing tunnel %s://%s(%s):%s <> %s:%s',
      localProtocol,
      localHost,
      localPort,
      remoteHostOrIp,
    ...
    const connLocal = () => {
      ...
      debug('connecting locally to %s://%s(%s):%d', localProtocol, localHost, localAddress, localPort);
      remote.pause();
      ...
      const local = opt.local_https
        ? tls.connect({ host: localAddress, port: localPort, ...getLocalCertOpts() })
        : net.connect({ host: localAddress, port: localPort });
