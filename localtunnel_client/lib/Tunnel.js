...
module.exports = class Tunnel extends EventEmitter {
  ...
  _getInfo(body) {
    ...
    const { id, ip, port, url, cached_url, max_conn_count, real_ip } = body;
    const { host, port: local_port, local_host, local_address } = this.opts;
  ...
  _init(cb) {
    ...
    if (opt.jwt) {
      const jwToken = fs.readFileSync(opt.jwt).toString().trim();
      params.headers = {
      ...
      axios
        ...
        .catch(err => {
          if(err.response.status === 401){
            return cb(`401 ${err.response.statusText}`);
          }
          const error = new Error(
            (err.response?.data && err.response.data?.message) ? err.response.data.message : 'The server returned an error, please try again'
          );
          return cb(error);
        });
    })();
